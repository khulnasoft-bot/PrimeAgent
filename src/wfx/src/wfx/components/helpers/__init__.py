...
if TYPE_CHECKING:
    ...
    from wfx.components.helpers.create_list import CreateListComponent
    from wfx.components.helpers.current_date import CurrentDateComponent
    from wfx.components.helpers.id_generator import IDGeneratorComponent
    from wfx.components.helpers.memory import MemoryComponent
    from wfx.components.helpers.output_parser import OutputParserComponent
    from wfx.components.helpers.store_message import MessageStoreComponent
...
# This allows imports like: import wfx.components.helpers.current_date
_redirected_submodules = {
    "wfx.components.helpers.current_date": "wfx.components.utilities.current_date",
    "wfx.components.helpers.calculator_core": "wfx.components.utilities.calculator_core",
    "wfx.components.helpers.id_generator": "wfx.components.utilities.id_generator",
    "wfx.components.helpers.memory": "wfx.components.models_and_agents.memory",
}
...
def __getattr__(attr_name: str) -> Any:
    ...
    # e.g., wfx.components.helpers.id_generator -> wfx.components.utilities.id_generator
    if attr_name == "id_generator":
        from importlib import import_module

        result = import_module("wfx.components.utilities.id_generator")
        globals()[attr_name] = result
        return result
    if attr_name == "calculator_core":
        from importlib import import_module

        result = import_module("wfx.components.utilities.calculator_core")
        globals()[attr_name] = result
        return result
    if attr_name == "current_date":
        from importlib import import_module

        result = import_module("wfx.components.utilities.current_date")
        globals()[attr_name] = result
        return result
    if attr_name == "memory":
        from importlib import import_module

        result = import_module("wfx.components.models_and_agents.memory")
        globals()[attr_name] = result
        return result
    ...
    if attr_name in ("CurrentDateComponent", "CalculatorComponent", "IDGeneratorComponent"):
        from wfx.components import utilities
