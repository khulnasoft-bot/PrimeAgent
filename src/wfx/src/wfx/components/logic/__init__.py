...
# This allows imports like: import wfx.components.logic.listen
_redirected_submodules = {
    "wfx.components.logic.listen": "wfx.components.flow_controls.listen",
    "wfx.components.logic.loop": "wfx.components.flow_controls.loop",
    "wfx.components.logic.notify": "wfx.components.flow_controls.notify",
    "wfx.components.logic.pass_message": "wfx.components.flow_controls.pass_message",
    "wfx.components.logic.conditional_router": "wfx.components.flow_controls.conditional_router",
    "wfx.components.logic.data_conditional_router": "wfx.components.flow_controls.data_conditional_router",
    "wfx.components.logic.flow_tool": "wfx.components.flow_controls.flow_tool",
    "wfx.components.logic.run_flow": "wfx.components.flow_controls.run_flow",
    "wfx.components.logic.sub_flow": "wfx.components.flow_controls.sub_flow",
}
...
def __getattr__(attr_name: str) -> Any:
    ...
    if attr_name == "listen":
        ...
        result = import_module("wfx.components.flow_controls.listen")
        globals()[attr_name] = result
        return result
    if attr_name == "loop":
        from importlib import import_module

        result = import_module("wfx.components.flow_controls.loop")
        globals()[attr_name] = result
        return result
    if attr_name == "notify":
        from importlib import import_module

        result = import_module("wfx.components.flow_controls.notify")
        globals()[attr_name] = result
        return result
    if attr_name == "pass_message":
        from importlib import import_module

        result = import_module("wfx.components.flow_controls.pass_message")
        globals()[attr_name] = result
        return result
    if attr_name == "conditional_router":
        from importlib import import_module

        result = import_module("wfx.components.flow_controls.conditional_router")
        globals()[attr_name] = result
        return result
    if attr_name == "data_conditional_router":
        from importlib import import_module

        result = import_module("wfx.components.flow_controls.data_conditional_router")
        globals()[attr_name] = result
        return result
    if attr_name == "flow_tool":
        from importlib import import_module

        result = import_module("wfx.components.flow_controls.flow_tool")
        globals()[attr_name] = result
        return result
    if attr_name == "run_flow":
        from importlib import import_module

        result = import_module("wfx.components.flow_controls.run_flow")
        globals()[attr_name] = result
        return result
    if attr_name == "sub_flow":
        from importlib import import_module

        result = import_module("wfx.components.flow_controls.sub_flow")
        globals()[attr_name] = result
        return result
    ...
    if attr_name in (
        ...
        from wfx.components import flow_controls
    ...
    if attr_name == "SmartRouterComponent":
        from wfx.components import llm_operations
