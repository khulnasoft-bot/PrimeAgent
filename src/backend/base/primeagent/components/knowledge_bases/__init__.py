"""Primeagent knowledge bases module - forwards to wfx.components.files_and_knowledge.
...
from wfx.components.files_and_knowledge import __all__ as _wfx_all

__all__: list[str] = list(_wfx_all)

# Register redirected submodules in sys.modules for direct importlib.import_module() calls
# This allows imports like: import primeagent.components.knowledge_bases.ingestion
_redirected_submodules = {
    "primeagent.components.knowledge_bases.ingestion": "wfx.components.files_and_knowledge.ingestion",
    "primeagent.components.knowledge_bases.retrieval": "wfx.components.files_and_knowledge.retrieval",
}
...
def __getattr__(attr_name: str) -> Any:
    """Forward attribute access to wfx.components.files_and_knowledge."""
    # Handle submodule access for backwards compatibility
    if attr_name == "ingestion":
        from importlib import import_module

        result = import_module("wfx.components.files_and_knowledge.ingestion")
    ...
    from wfx.components import files_and_knowledge

    return getattr(files_and_knowledge, attr_name)


def __dir__() -> list[str]:
    """Forward dir() to wfx.components.files_and_knowledge."""
    return list(__all__)
